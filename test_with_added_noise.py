...
#     min_length = min(len(clean_speech), len(processed_speech))
#     clean_speech = clean_speech[:min_length]
#     processed_speech = processed_speech[:min_length]
...
model = keras.models.load_model('/work/pbenga2s/Pytorch/thesis/lunet/saved_models/5data/lunet_model_pc07_PhysioNet_1000_-3dB_to_6dB_(5data).h5') 
...
# output_directory = '/work/pbenga2s/Pytorch/thesis/lunet/Output_of_5datatrained/dog_data/dog_noise_data(5snr)/'
# spectrogram_directory = '/work/pbenga2s/Pytorch/thesis/lunet/Output_of_5datatrained/dog_data/dog_noise_data(5snr)/specto'
# snr_mse_results_file = os.path.join(output_directory, '/work/pbenga2s/Pytorch/thesis/lunet/Output_of_5datatrained/dog_data/dog_noise_data(5snr)/snr_results.txt')
desired_snr_db = 1  # Adjust noise levels as needed

# input_directory = '/work/pbenga2s/Pytorch/thesis/lunet/Data/oahs(all_in_one)'  
# # output_directory = '/work/pbenga2s/Pytorch/thesis/lunet/Output_of_5datatrained/human_data/human_noise_data(-5snr)/'
# # spectrogram_directory = '/work/pbenga2s/Pytorch/thesis/lunet/Output_of_5datatrained/human_data/human_noise_data(-5snr)/specto'
# # snr_mse_results_file = os.path.join(output_directory, '/work/pbenga2s/Pytorch/thesis/lunet/Output_of_5datatrained/human_data/human_noise_data(-5snr)/snr_results.txt')

# output_directory = '/work/pbenga2s/Pytorch/thesis/lunet/Output_of_5datatrained/human_data_lp/human_noise_data(1snr)/'
# spectrogram_directory = '/work/pbenga2s/Pytorch/thesis/lunet/Output_of_5datatrained/human_data_lp/human_noise_data(1snr)/specto'
# snr_mse_results_file = os.path.join(output_directory, '/work/pbenga2s/Pytorch/thesis/lunet/Output_of_5datatrained/human_data_lp/human_noise_data(1snr)/snr_results.txt')
...
# desired_snr_db = 2  # Adjust noise levels as needed
...
#     file.write("Filename, Noise Added SNR (r-n), Model Processed SNR (n-d)(dB), Noise Reduction SNR (r-d)(dB), Noise Added MSE (r-n), Model Processed MSE (n-d), Alternative Noise Reduction (r-d), noise ssnr, model ssnr, org ssnr\n")
...
#             #     add_electronic_interference(signal, desired_snr_db, sr, 50) +\
#             #     add_cable_movement_noise(signal, desired_snr_db, 0.3, 0.3, sr)
#             org_signal = signal
...
#             noise_added_ssnr = SNRseg(signal,noisy_pcg_all_noises,sr)
...
#             combined_signal = combine_segments(processed_segments, sr, overlap=0)


#             # Calculate the Power Spectral Density (PSD) of the original signal
#             # frequencies, power_density = signal.welch(org_signal, sr, nperseg=1024)
#             # frequencies, power_density = scipy_signal.welch(org_signal, sr, nperseg=1024)
#             # # Determine the frequency below which a certain percentage of the signal's power lies
#             # # For example, to retain 90% of the signal's power
#             # power_cumsum = np.cumsum(power_density)
#             # power_total = power_cumsum[-1]
#             # cutoff_idx = np.where(power_cumsum >= 0.95 * power_total)[0][0]
#             # cutoff_frequency = frequencies[cutoff_idx]
#             # # Print out the cutoff frequency
#             # print(f"The cutoff frequency to retain 90% of the power is: {cutoff_frequency} Hz")
#             # # Design the low-pass filter
#             # nyquist = 0.5 * sr
#             # norm_cutoff = cutoff_frequency / nyquist
#             # b, a = scipy_signal.butter(N=5, Wn=norm_cutoff, btype='low', analog=False)
#             # # Apply the filter to the combined_signal
#             # filtered_signal = scipy_signal.filtfilt(b, a, combined_signal)
...
#             model_processed_ssnr = SNRseg(signal,combined_signal,sr)

#             org1 = estimate_snr(org_signal, combined_signal)
#             org2 = calculate_mse(org_signal, combined_signal)

#             org3 = SNRseg(org_signal,combined_signal,sr)
...
#             # file.write(f"{filename}, {noise_added_snr}, {model_processed_snr}, {pynr_processed_snr}, {noise_added_mse}, {model_processed_mse}, {pynr_processed_mse}\n")
#             # file.write(f"{filename}, {noise_added_snr}, {model_processed_snr}, {org1}, {noise_added_mse}, {model_processed_mse}, {org2}\n")
#             file.write(f"{filename}, {noise_added_snr}, {model_processed_snr}, {org1}, {noise_added_mse}, {model_processed_mse}, {org2}, {noise_added_ssnr}, {model_processed_ssnr}, {org3}\n")
...
input_directory = '/work/pbenga2s/Pytorch/thesis/lunet/Data/noise_data/data/Files'  
output_directory = '/work/pbenga2s/Pytorch/thesis/lunet/Output_of_5datatrained/dog_data/without_noise/'
spectrogram_directory = '/work/pbenga2s/Pytorch/thesis/lunet/Output_of_5datatrained/dog_data/without_noise/specto'
snr_mse_results_file = os.path.join(output_directory, '/work/pbenga2s/Pytorch/thesis/lunet/Output_of_5datatrained/dog_data/without_noise/snr_results.txt')
